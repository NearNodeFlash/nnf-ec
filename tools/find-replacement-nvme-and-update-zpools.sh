...
get_zpool_devices() {
    ...
    while read -r line; do
        ...
        if [[ "$device" =~ ^[0-9]+$ ]]; then
            # This is a numeric vdev ID - keep it as is without adding /dev/ prefix
            full_path_device="$device"
            echo "Detected numeric vdev ID: $device" >&2
        elif [[ "$device" == /* ]]; then
        ...
        local device_to_check="$full_path_device"
        # Only try to check physical existence for /dev/ paths, not numeric vdevs
        if [[ "$device_to_check" =~ ^[0-9]+$ ]]; then
            # Can't check physical existence of numeric vdev
            if [[ "$state" == "UNAVAIL" || "$state" == "OFFLINE" || "$state" == "REMOVED" ||
                  "$state" == "FAULTED" || "$state" == "DEGRADED" ]]; then
                # Device is not healthy - add to missing (needs replacement)
                missing_devices+=("$full_path_device")
            else
                # Device appears to be healthy
                present_devices+=("$full_path_device")
            fi
        elif [[ -b "$device_to_check" ]]; then
...
replace_zpool_device() {
    ...
    # Handle the case where old_device is a numeric ID
    if [[ "$old_device" =~ ^[0-9]+$ ]]; then
        echo "DEBUG: Executing replace command: $replace_cmd $pool $old_device $new_device" >&2
        echo "Replacing device in pool '$pool': using numeric vdev ID '$old_device' with '$new_device'"
    ...
    while read -r line; do
        ...
        if [[ "$state" == "UNAVAIL" || "$state" == "FAULTED" || "$state" == "DEGRADED" ]]; then
            ...
            if $DRY_RUN; then
                ...
                if zpool offline "$pool" "$device"; then
                    offlining_done=true
                else
...
# If OFFLINE_ALL mode is specified, run that directly
if $OFFLINE_ALL; then

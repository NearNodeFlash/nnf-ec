...
#                The function takes a Chassis# (0-7) and slot# (4 or 7) as arguments
...
function PowerUpNode {
    ...
    if ! (cm power status -n x1002c$((Chassis_num))j7b0n0 | grep --quiet 'BOOTED'); then
        ...
        echo -e "\n\nRabbit node PXE-booting, this will take 510 minutes . . .\n\n"
        sleep 5m
...
# Update the nC FW if not currently 1.8.313. This FW revision has necessary fixes for Rabbit.
if ! (ssh x1002c$((Chassis_num))j4b0 cat /usr/etc/version/hms-controllers.version | grep '1.8.313'); then
    echo "Problem: Rabbit-S nC is down-rev, need to Flash it!"  | tee -a "$(pwd)"/logs/Chassis$((Chassis_num)).log
    scp "$(pwd)"/FwImages/nc1.8.313.itb x1002c$((Chassis_num))j4b0:/root/
...
if [ "$had2flash" == "yes" ] ; then
    ...
    echo -e "     Rabbit-S nC FW is v1.8.313!"  | tee -a "$(pwd)"/logs/Chassis$((Chassis_num)).log
...
# Update the nC FW if not currently 1.8.313. This FW revision has necessary fixes for Rabbit.
if ! (ssh x1002c$((Chassis_num))j7b0 cat /usr/etc/version/hms-controllers.version | grep '1.8.313'); then
    echo "Problem: Rabbit-P nC is down-rev, need to Flash it!"  | tee -a "$(pwd)"/logs/Chassis$((Chassis_num)).log
    scp "$(pwd)"/FwImages/nc1.8.313.itb x1002c$((Chassis_num))j7b0:/root/
...
if [ "$had2flash" == "yes" ] ; then
    ...
    echo -e "     Rabbit-P nC FW is v1.8.313!"  | tee -a "$(pwd)"/logs/Chassis$((Chassis_num)).log
...
#    re-verify that all 16 drives were found. . .
   ...
   declare -i num_drv_pax0
   num_drv_pax0=$(cat "$(pwd)"/logs/Chassis$((Chassis_num))_pax0.log | grep -c -F 'neg[x04]')
   ...
   declare -i num_drv_pax1
   num_drv_pax1=$(cat "$(pwd)"/logs/Chassis$((Chassis_num))_pax1.log | grep -c -F 'neg[x04]')
...
if ! (ssh x1002c$((Chassis_num))j7b0n0 dmidecode -t 17 | grep -E 'Size: 32' | grep -E -v V |wc -l | grep -E 8); then
    DATE_TIME=$(date '+%Y-%m-%d %H:%M:%S')                            # Date/time stamp for the log file
...
declare -i num_nics
num_nics=$(ssh x1002c$((Chassis_num))j7b0n0 cxi_stat | grep -c 'cxi')
...
declare -i num_200Gb
num_200Gb=$(ssh x1002c$((Chassis_num))j7b0n0 cxi_stat | grep -c 'Link speed: BS_200G')

...
if __name__ == '__main__':
    ...
    print(f'Beginning Storage Pool and Group Create/Delete Loop: Endpoint: {endpoint_id}')
    while True:
        # Storage Pool - Create
        payload = {
        ...
        print(f'Creating Storage Pool: Size: {args.size}...', end='', flush=True)
        ...
        pauseInSeconds = 5.0
        print(f'Pause {pauseInSeconds} seconds for Storage Group to come ready')
        ...
        # print(f'Pause {pauseInSeconds} seconds for Storage Group to delete')
        # time.sleep(pauseInSeconds)

        # Storage Pool - Delete
        print(f'Delete Storage Pool {pool_id}....', end='', flush=True)
        response = conn.delete(f'/StoragePools/{pool_id}')
        if not response.ok:
            print(f'Storage Pool Delete: Error: {response.status_code}')
            sys.exit(0)
        print(f'Deleted: Id: {pool_id}')

        print(f'Pause {pauseInSeconds} seconds for before starting next loop')
        time.sleep(pauseInSeconds)
